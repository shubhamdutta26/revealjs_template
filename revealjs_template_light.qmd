...
## Fit 4-parameter log-logistic (4PL) model
fit <- drm(mean_od450 ~ conc_ug, data = summary_qc, fct = LL.4())

## Extract and rename parameters
pars <- coef(fit)
names(pars) <- c("hill_slope", "bottom", "top", "EC50")

ec50 <- pars["EC50"]
ec50_ci <- ED(fit, 50, interval = "delta") # EC50 + CI

## y value at EC50 (half-max between top & bottom)
y_ec50 <- pars["bottom"] + (pars["top"] - pars["bottom"]) / 2
```

```{r}
#| label: elisa-plot
ggplot(data = summary_qc, aes(x = conc_ug, y = mean_od450)) +
  geom_smooth(
    method = drm,
  ...
  ## EC50 segments from axes to point
  geom_segment(
    aes(x = 0, y = y_ec50, xend = ec50, yend = y_ec50),
    linetype = "dashed",
    linewidth = 0.8,
    colour = "grey40",
    inherit.aes = FALSE
  ...
  geom_segment(
    aes(x = ec50, y = 0, xend = ec50, yend = y_ec50),
    linetype = "dashed",
    linewidth = 0.8,
    colour = "green",
    inherit.aes = FALSE
  ) +
  ## Optional label near the intersection
  annotate(
    "text",
    x = ec50,
    y = y_ec50,
    label = paste0("EC50 = ", signif(ec50, 3)),
    hjust = -0.1,
    vjust = -0.5,
    size = 3
  ) +
